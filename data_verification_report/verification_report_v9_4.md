# v9.4 검증 보고서 [26.09.26 재확인]

## — Claim 문장·subject/object URI·relation 의미 정합성 검증

> 본 보고서는 「v8.5 검증 보고서(=ver9 설계 승인본)」의 형식을 준용했다.
> v9.3에서 구조적으로는 완결됐으나 의미론적 정합성이 남아 있던 문제를 실제 파일 대조를 통해 검토·수정한 내역으로 v9.4를 만들었고, 이 v9.4 내역을 보고서를 통해 정리한다.
...
- **목적**: v9.3까지의 검증이 행 수·CURIE 형식·날짜 형식 등 기계적으로 확인 가능한 정합성에 머물렀던 것을, Claim 문장이 실제 `subject_uri → relation → object_uri` 방향과 일치하는가라는 **의미론적 정합성**까지 확장한다.

> 참고> v9.5와 v8 그리고 v5 csv 파일을 제외한 나머지는 전부 로컬에서 관리하고, git에서는 생략하기에 기존 버전은 git commit 통해 확인해야 한다.

---

## 1. 문제의 성격 — 형식은 맞지만 의미가 어긋나는 경우

v9.3까지의 검증은 CURIE 형식이나 날짜 포맷처럼 구문(syntax) 수준에서 확인 가능한 항목을 대상으로 했다. 실제 파일을 행 단위로 다시 읽은 결과, `claim` 자연어 문장과 `subject_uri`·`object_uri`·`relation` 사이에 의미론적 불일치가 발견됐다.

이러한 문제는 필드 형식만 검사하는 스크립트로는 충분히 검출하기 어렵다. 문장의 실제 주어와 관계 방향, 대상 엔티티가 구조화된 필드와 일치하는지를 사람이 직접 검토해야 한다.

> 참고> CURIE는 "Compact URI"(압축 URI) 즉, "Compact URI Expression"의 약자다.
> prefix:localname 형태로 URI를 줄여 쓰는 표기법(예: :이승훈처럼 :가 https://ontology.gwanhak-inmul.kr/entity/ 같은 전체 네임스페이스의 축약 접두어 역할)을 가리킨다.
...
- subject 엔티티가 원본 인물 매핑과 다르다는 사실은 `change_history`에 기록
-  (=> 정정 9.4 자체를 실패본으로 간주하고, 이 문제를 그냥 주어 문제로 여겨 자세히 기록 안 함 [26.09.23])
> 참고로 여기서의 change_history는 v9.x.ttl 내부에 사용되는 트리플(Triple)을 의미한다.
> git commit 이력을 보면 verification_report_v1-v10.md을 'change_history'라고도 불렀다는 게 나오는데,
> RDF에서의 트리플(명재)보다는 그냥 'change_history'라는 말 자체에 집중해주길 바란다.
...
Claim 문장을 "이회영 등이 신흥강습소를 설립하였다"로 바꿔 구조화된 방향에 맞췄다. 
공동설립자 전원을 주어에 넣으면 다중 주체 문제가 생길 수 있으므로 "등"으로 대표 표기했다. 
개별 공동설립자별 Claim은 후속 확장 과제로 남겼다.
...
A안을 채택했다. "서일·"을 제거하고 "북로군정서는 김좌진이 주도한 독립군 조직이다"로 정리했다. 
서일과 북로군정서의 관계는 별도 Claim인 CLM-022a(item 43)가 다루고 있으므로, 
해당 문장에서 서일을 제거해도 이 관계를 다루는 Claim 자체가 사라지는 것은 아니다.
...
(기존 트리플 자리에 성명을 복사 붙여넣기하는 과정에서 조사 오류가 발생한 듯하다.)
---
...
## 5. 자동검증 8개 
아래 영역을 기본 검증으로 두고, 매 회차 검증을 돌렸다.
검증은 README에 명시한 것처럼 AI(주로 SONNET5)와 병행했고, 보안 등 기본 요소를 중시했다.
...
> (정정> DS NLP 과정처럼 dict을 만든 건데, 너무 복잡해져서 dict 방식 폐기했다. 그럼에도 이런 시도가 있었음을 남기고자 v9_4를 저장소에 그대로 놔둔 것이다.)
...
> 참고> 공훈 api 스냅샷 남기는 방식가 비슷하게 여기서도 스냅샷으로 작업을 연결하려 했는데, (정확하게는 dict과 ttl 작업을 연결하려 했는데),  너무 복잡해져서 그냥 폐기했다. 
> 시점 명시 등 말이 쉽지, 스냅샷 방식으로 진행하면 고려해야 할 '증거 자료'만 늘어나게 된다.
...
이는 v8 검증보고서 ver5 §9의 Schema/Source/Evidence/Claim/Date/Relation/Existence/Provenance validation 구분과 대응한다. v9.4는 그중 Relation validation, 
> **즉 subject/object 관계가 실제로 맞는지에 대한 검토를 본격적으로 수행한 단계**로 볼 수 있다.

### 부록. v8.5 검증보고서(: v9.x 설계승인본) 조항과의 대응

| v8.5 보고서 조항 | v9.4에서의 적용 |
|---|---|
...
데이터 사전의 실제 Claim 예시는 v9.4 릴리스 시점에 한정된 스냅샷이다. 후속 릴리스의 CSV와 필드값이 다를 경우, 릴리스 간 변경인지 문서 갱신 누락인지 구분해 기록한다. 이 원칙은 v9.5에서 Rule P-05로 구체화한다. =>라고 적었으나, 이 방식 자체를 폐기했다.
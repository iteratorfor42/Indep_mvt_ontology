...
본문에서 내가 실제로 확인한 고유 URL 33건을, 발행처별·항목명 가나다순으로 직접 정리했다. 두 발행처는 각각 **한국학중앙연구원**(한국민족문화대백과사전 운영기관, encykorea.aks.ac.kr)과 **국사편찬위원회**(우리역사넷 운영기관, contents.history.go.kr)이다. 나는 각 URL의 발행처를 도메인 기준으로 확인해 저자명을 확정했으며, 접속일은 2026년 8월 29일 기준으로 기록했다.
...
**비고**: 나는 표 5(인물 30명) 및 표 A(학교·단체)에서 "재확인 필요"로 표시한 항목(숭실학교, 보성학교, 신간회, 함석헌, 배위량, 차이석, 이용익, 이동녕, 이상룡, 지청천, 이범석, 박희도, 청산리전투)을 이 참고문헌 목록에는 포함하지 않았다. 기존 CSV에는 URL이 존재한다고 되어 있었지만, 내가 직접 확인한 원문에는 실제 URL 문자열이 제시되지 않았기 때문이다. 나는 이런 경우 가짜 URL을 만들어 채우지 않는다는 Rule P-01 원칙을 그대로 적용했다. 해당 13건은 본문 12절에서 내가 "URL은 존재하지만 세부 주장까지 직접 검증하지 못한 항목"으로 남겨둔 상태이며, v9 작업 시 내가 실제 URL을 확보한 뒤 동일한 APA 형식으로 추가할 계획이다.
